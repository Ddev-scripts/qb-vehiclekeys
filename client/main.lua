    ...
    if veh then
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) then
                ...
                -- Decors:
                KINGSTOWN_GetControlOfEntity(veh)

                if not DecorExistOn(veh, lock_decor) then
                    SetVehicleLocked(veh, GetVehicleDoorLockStatus(veh))
                end

                NetworkRequestControlOfEntity(veh)


                if DecorGetInt(veh, lock_decor) == 1 or DecorGetInt(veh, lock_decor) == 0 then
                    SetVehicleLocked(veh, Config.Lock.LockInt)
                    QBCore.Functions.Notify(Lang:t("notify.vlock"), "primary")
                elseif DecorGetInt(veh, lock_decor) == 2 or DecorGetInt(veh, lock_decor) == 10 then
                    SetVehicleLocked(veh, Config.Lock.UnlockInt)
                    QBCore.Functions.Notify(Lang:t("notify.vunlock"), "success")
                end

                SetVehicleDoorsLocked(veh, DecorGetInt(veh, lock_decor))
...
-------------------------------- CUSTOM -------------------------------------
-----------------------------------------------------------------------------
function KINGSTOWN_GetControlOfEntity(entity)
    local netTime = 15
...
function SetVehicleLocked(vehicle, int)
    if vehicle ~= 0 and DoesEntityExist(vehicle) then
        KINGSTOWN_GetControlOfEntity(vehicle)
        local integer = 0
